...
expect "patterns are saved in order" $(test "${ROUTE_PATTERN[1]}" = "/\?two")
...
expect "get helper saves correct pattern" $(test "${ROUTE_PATTERN[2]}" = "/\?get")
...
expect "post helper saves correct pattern" $(test "${ROUTE_PATTERN[3]}" = "/\?post")
...
expect "put helper saves correct pattern" $(test "${ROUTE_PATTERN[4]}" = "/\?put")
...
expect "patch helper saves correct pattern" $(test "${ROUTE_PATTERN[5]}" = "/\?patch")
