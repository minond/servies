...
expect "extractors are saved in order" $(test "${ROUTE_EXTRACTOR[1]}" = "s/\/{0,}two\/{0,}(\w+)\/{0,}(\w+)/ \1 \2/")
...
expect "get helper saves correct extractors" $(test "${ROUTE_EXTRACTOR[2]}" = "s/\/{0,}get\/{0,}(\w+)\/{0,}(\w+)/ \1 \2/")
...
expect "post helper saves correct extractors" $(test "${ROUTE_EXTRACTOR[3]}" = "s/\/{0,}post\/{0,}(\w+)\/{0,}(\w+)/ \1 \2/")
...
expect "put helper saves correct extractors" $(test "${ROUTE_EXTRACTOR[4]}" = "s/\/{0,}put\/{0,}(\w+)\/{0,}(\w+)/ \1 \2/")
...
expect "patch helper saves correct extractors" $(test "${ROUTE_EXTRACTOR[5]}" = "s/\/{0,}patch\/{0,}(\w+)\/{0,}(\w+)/ \1 \2/")
